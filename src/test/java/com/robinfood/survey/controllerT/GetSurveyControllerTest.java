...
public class GetSurveyControllerTest {
    ...
    @Test
    public void GetSurveyTestFail() throws Exception {
        ...
        MvcResult resultGetSurvey = mvc.perform(
                        MockMvcRequestBuilders.post("/survey")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(questionDtoList)))
                .andExpect(status().isCreated())
                .andReturn();
        String resultado = resultGetSurvey.getResponse().getContentAsString();
        Assertions.assertEquals("Added Questions id = 1", resultado);

        MvcResult resultGetSurvey2 = (MvcResult) mvc.perform(MockMvcRequestBuilders
                        .get("/survey/20")
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isNotFound()).andReturn();
        String resultado2 = resultGetSurvey2.getResponse().getContentAsString();
        Assertions.assertEquals(NOT_FOUND_SURVEY, resultado2);
    }
}
